...
desc = df_all.groupby('Algorithm').agg(
...
# ===== Boxplots =====
plt.figure()
df_all.boxplot(column='Moves', by='Algorithm', rot=45)
plt.title('Boxplot de Moves por Algoritmo')
plt.suptitle('')
plt.ylabel('Moves')
plt.tight_layout()
plt.show()
...
plt.figure()
df_all.boxplot(column='Time', by='Algorithm', rot=45)
plt.title('Boxplot de Time por Algoritmo')
plt.suptitle('')
plt.ylabel('Time (s)')
plt.tight_layout()
plt.show()
...
for alg in df_all['Algorithm'].unique():
    if alg != 'BFS':
        ...
        plt.scatter(subset['Moves'], subset['Time'], label=alg)
plt.xlabel('Moves')
plt.ylabel('Time (s)')
plt.title('Scatter Moves vs. Time')
plt.legend()
plt.tight_layout()
plt.show()

# ===== Bar charts de média ± desvio =====
# Moves
plt.figure()
plt.bar(desc.index, desc['Moves_mean'])
plt.errorbar(desc.index, desc['Moves_mean'], yerr=desc['Moves_std'], fmt='none', capsize=5)
plt.xticks(rotation=45)
plt.title('Média de Moves ± 1 desvio-padrão')
plt.tight_layout()
plt.show()

# Time
plt.figure()
plt.bar(desc.index, desc['Time_mean'])
plt.errorbar(desc.index, desc['Time_mean'], yerr=desc['Time_std'], fmt='none', capsize=5)
plt.xticks(rotation=45)
plt.title('Média de Time ± 1 desvio-padrão')
plt.tight_layout()
plt.show()

# ===== Taxa de Sucesso =====
plt.figure()
plt.bar(desc.index, desc['Success_rate'])
plt.xticks(rotation=45)
plt.ylabel('Sucesso (%)')
...
# ===== Correlação Pearson =====
print("\n=== Correlação Pearson (Moves × Time) por Algoritmo ===")
for alg in df_all['Algorithm'].unique():
    subset = df_all[df_all['Algorithm'] == alg]
    corr, p_value = pearsonr(subset['Moves'], subset['Time'])
    print(f"{alg}: correlação = {corr:.2f}, p-value = {p_value:.3f}")

# ===== Testes t entre heurísticas =====
def run_tests(a1, a2, metric):
    t_stat, p_val = ttest_ind(a1, a2, equal_var=False)
    print(f"{metric}: t-statistic = {t_stat:.2f}, p-value = {p_val:.3f}")

print("\n=== Testes t: SA Manhattan vs SA Pieces_out_of_place ===")
sa_m = df_all[df_all['Algorithm'] == 'SA_Manhattan']
sa_p = df_all[df_all['Algorithm'] == 'SA_Pieces_out_of_place']
run_tests(sa_m['Moves'], sa_p['Moves'], 'Moves')
run_tests(sa_m['Time'], sa_p['Time'], 'Time')

print("\n=== Testes t: HC Manhattan vs HC Pieces_out_of_place ===")
hc_m = df_all[df_all['Algorithm'] == 'HC_Manhattan']
hc_p = df_all[df_all['Algorithm'] == 'HC_Pieces_out_of_place']
run_tests(hc_m['Moves'], hc_p['Moves'], 'Moves')
run_tests(hc_m['Time'], hc_p['Time'], 'Time')

# ===== Análise de falhas (HC Manhattan) =====
print("\n=== Instâncias com falha (HC_Manhattan) ===")
failures = hc_m[hc_m['Result'] == 'D']
print(failures[['Moves', 'Time']])